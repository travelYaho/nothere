...
def test_build_guide_includes_saved_itinerary_fields(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.domains.recommendation.service.photo_gallery.image_for_place",
        lambda _place: None,
    )
    monkeypatch.setattr(
    ...
    assert stop["imageUrl"] is None
    assert result["cityName"] == "서울"
    assert result["districtName"] == "종로구"
    assert result["totalTravelMin"] == 0
    assert result["memo"] is None
    assert result["tags"] == []


def test_build_guide_resolves_photos_and_memo(monkeypatch):
    db = MagicMock()
    svc = RecommendationService(db)
    place_id = uuid4()
    tp = SimpleNamespace(
...
def test_update_guide_memo_upserts_for_owner():
    ...
    svc.repo.get_trip_owned = MagicMock(return_value=trip)
    svc.repo.upsert_trip_memo = MagicMock()

    result = svc.update_guide_memo(trip_id, user, "  여행 메모  ")

    assert result == {"memo": "여행 메모"}
    svc.repo.upsert_trip_memo.assert_called_once_with(trip_id, "여행 메모")
    db.commit.assert_called_once()


def test_update_guide_memo_rejects_unconfirmed():
    db = MagicMock()
    svc = RecommendationService(db)
    user = _user()
    trip_id = uuid4()
    svc.repo.get_trip_owned = MagicMock(
        return_value=SimpleNamespace(id=trip_id, user_id=user.id, status="draft")
    )

    with pytest.raises(AppError) as exc:
        svc.update_guide_memo(trip_id, user, "메모")
